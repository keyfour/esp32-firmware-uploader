...
class TestStorageMethods(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.valid_json_path = self.path_prefix.join('/valid.json')
    self.invalid_json_path = self.path_prefix.join('/invalid.json')

  def test_local_json_storage_paths(self):
    self.assertEqual(self.path_prefix.join('/boards.json'),
      self.local_json_storage.boards_file)
    self.assertEqual(self.path_prefix.join('/frameworks.json'),
      self.local_json_storage.frameworks_file)

  def test_local_json_storage_parse_file(self):
    with open(self.valid_json_path) as f:
      f.write(self.valid_json)
    with open(self.invalid_json_path) as f:
    ...
    self.assertEqual(result[0]['board'], "ESP32")
    self.assertEqual(result[0]['name'], "Device")
